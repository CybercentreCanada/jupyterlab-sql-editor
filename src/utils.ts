...
function line_magic(language: string) { return `%${language}` }
function cell_magic(language: string) { return `%%${language}` }
function start(language: string) { return `--start-${language}` }
function end(language: string) { return `--end-${language}` }
...
export function sqlCodeMirrorModesFor(language: string, sqlMode: Mode<unknown>) {
    return [
        {
            open: `${start(language)}`,
            close: `${end(language)}`,
...
export function lineMagicExtractor(language: string): RegExpForeignCodeExtractor {
...
export function cellMagicExtractor(language: string): RegExpForeignCodeExtractor {
    return new RegExpForeignCodeExtractor({
        language: language,
 ...
 export function registerCodeMirrorFor(c: ICodeMirror, language: string) {
    c.CodeMirror.defineMode(
      ...
      (config: CodeMirror.EditorConfiguration, modeOptions?: any) => {
        var mode = c.CodeMirror.getMode(config, 'sql')
        return mode
      }
    )
